...
GRANT
  ...
  DELETE ON TABLE graph_editor.graph TO graph_editor_person;
...
BEGIN
  SELECT a.* INTO account
  FROM graph_editor.account a, graph_editor.person p
  ...
  IF (account.password_hash = crypt($2, account.password_hash)) THEN
    DELETE FROM graph_editor.person WHERE id = account.person_id;
  ELSE
    RAISE EXCEPTION 'wrong-password';
  END IF;
...
GRANT EXECUTE ON FUNCTION graph_editor.remove_person(text, text) TO graph_editor_person;
